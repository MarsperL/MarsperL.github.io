...
const matchCacheRule = (url) => {
        ...
        // 仅仅对于yyg.js.cool 处理 html 和 json
        if (host.endsWith('yyg.js.cool')) {
        ...
        // return ((url.host.endsWith('yyg.js.cool') && /(\/|\.json)$/.test(url.pathname)) || /\.(js|css|woff2|woff|ttf|cur)$/.test(url.pathname)) ? 86400000 : false
      };
...
const getStandbyRequests = (request) => {
      ...
      const commonCdnList = ['s4.zstatic.net', 'cdn.jsdmirror.com', 'fastly.jsdelivr.net'];
      // noinspection SpellCheckingInspection
